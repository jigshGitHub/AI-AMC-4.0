...
=============================================
A ReAct agent built with LangGraph that connects to two MCP servers (Notes + Calculator) via langchain-mcp-adapters.
...
async def run_interactive_chat():
    ...
    while True:
        ...
        chat_config = {"configurable": {"thread_id": f"session-{session_id}"}}
        ...
        try:
            ...
            approved, denied = await handle_hitl_loop(graph, chat_config)

            final_msg = graph.get_state(chat_config).values["messages"][-1]
...
async def handle_hitl_loop(graph, graph_config: dict) -> tuple[list[str], list[str]]:
    """
    ...
    while True:
        ...
        if decision == "y":
            ...
            await graph.ainvoke(None, config=graph_config)
        else:
            for tc in pending_calls:
